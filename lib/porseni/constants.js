...
// ==========================================================================
// PERINGKAT KHUSUS CABANG OLAHRAGA "DUA JUARA 3" (juara 3 bersama), tanpa Harapan
// Cabang: Voli, Bulutangkis/Badminton, Futsal, Tenis Meja.
// ==========================================================================
// Key internal untuk juara 3 kedua (agar unik di backend: lomba_id+rank+gender),
// tetapi ditampilkan sebagai "Juara 3" di seluruh UI/sertifikat/Excel.
export const SECOND_BRONZE_RANK = 'Juara 3 Bersama'
export const DUAL_BRONZE_KEYWORDS = ['voli', 'bulu tangkis', 'bulutangkis', 'badminton', 'futsal', 'tenis meja']

// Deteksi cabang lomba yang memakai format dua juara 3 dari nama lomba.
export function usesDualBronze(lombaName = '') {
  const n = String(lombaName || '').toLowerCase().replace(/\s+/g, ' ').trim()
  return DUAL_BRONZE_KEYWORDS.some((k) => n.includes(k))
}

// Daftar peringkat (key internal + label tampil) sesuai cabang lomba.
// Voli/Bulutangkis/Futsal/Tenis Meja => Juara 1, 2, 3, dan 3 (bersama) tanpa Harapan.
export function ranksForLomba(lomba) {
  if (lomba && usesDualBronze(lomba.name)) {
    return [
      { key: 'Juara 1', label: 'Juara 1' },
      { key: 'Juara 2', label: 'Juara 2' },
      { key: 'Juara 3', label: 'Juara 3' },
      { key: SECOND_BRONZE_RANK, label: 'Juara 3' },
    ]
  }
  return RANKS.map((r) => ({ key: r, label: r }))
}

// Label tampil untuk sebuah rank: juara 3 kedua tampil "Juara 3".
export function rankDisplay(rank) {
  return rank === SECOND_BRONZE_RANK ? 'Juara 3' : (rank || '')
}
